...
model_name = 'pruned_simplenet.h5'

f = open('final_results_simplenet_pruned.txt','w')
...
sparsity.prune_low_magnitude((tensorflow.keras.layers.Conv2D(512, (3, 3), padding='same', activation='relu', input_shape=x_train.shape[1:], strides=(1, 1))), **pruning_params),
tensorflow.keras.layers.BatchNormalization(scale = True),  
tensorflow.keras.layers.Dropout(0.2),

# 11th Convolutional Layer
sparsity.prune_low_magnitude((tensorflow.keras.layers.Conv2D(2048, (1, 1), padding='same', activation='relu', input_shape=x_train.shape[1:], strides=(1, 1))), **pruning_params),
tensorflow.keras.layers.Dropout(0.2),
...
plt.savefig('accuracy_pruned_simplenet.png')
plt.show()
