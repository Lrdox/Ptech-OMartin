...
model_name = 'pruned_simplenetslim.h5'

f = open('final_results_pruned_simplenetslim.txt','w')
...
sparsity.prune_low_magnitude((tensorflow.keras.layers.Conv2D(32, (3, 3), padding='same', activation='relu', input_shape=x_train.shape[1:], strides=(1, 1))), **pruning_params),
tensorflow.keras.layers.BatchNormalization(scale = True),    
...
sparsity.prune_low_magnitude((tensorflow.keras.layers.Conv2D(128, (3, 3), padding='same', activation='relu', input_shape=x_train.shape[1:], strides=(1, 1))), **pruning_params),
tensorflow.keras.layers.BatchNormalization(scale = True),  

# 11th Convolutional Layer
sparsity.prune_low_magnitude((tensorflow.keras.layers.Conv2D(256, (1, 1), padding='same', activation='relu', input_shape=x_train.shape[1:], strides=(1, 1))), **pruning_params),
    
# 12th Convolutional Layer
sparsity.prune_low_magnitude((tensorflow.keras.layers.Conv2D(64, (1, 1), padding='same', activation='relu', input_shape=x_train.shape[1:], strides=(1, 1))), **pruning_params),
tensorflow.keras.layers.MaxPooling2D((2,2),(2,2), padding = 'valid'), 
...
callbacks = [pruning_callbacks.UpdatePruningStep(), pruning_callbacks.PruningSummaries(log_dir="saved_models"), PredictionCallback()]
...
plt.savefig('accuracy_pruned_simplenetslim.png')
plt.show()
